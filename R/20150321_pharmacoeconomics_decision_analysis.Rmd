...
## Introduction
You are interested in comparing the cost and outcomes over a 20 year time peroid of preventative
...
Now that we've computed that average LY give per treatment we can apply the same process to compute the avereage
utily for each treatment. Once we compute the average utily we can calculate the Quality Adjusted Life Years for
...
each treatment. This is done by multiplyting the expected LYs of each treatment with the average utility
...
We compute the incremental cost per LY gained according to the formulat below
...
In the plot above, we see that the incremental cost associated with Treatment B has a mean of about $50 with an incremental lifeyears of about 0.03 years. Also note that the 95% confidence ellipse extends beyond the cost and
lifeyear axes.
